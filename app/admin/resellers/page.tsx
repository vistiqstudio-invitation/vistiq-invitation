...
Terima kasih dan selamat mengembangkan bisnis undangan digital bersama kami! 🚀`;
  ...
  const extendBrandExpiry = async (reseller: Reseller, months: number) => {
    ...
    try {
      ...
      alert(`Order ${result.orderId} dibuat. Buka Owner → Transaksi untuk mengonfirmasi setelah transfer masuk.`);
    } finally {
            ...
            >
              ...
              {credentials.orderId && (
                <p style={{ margin: "4px 0 12px", color: "#1d4ed8" }}>
                  Order paket: <code>{credentials.orderId}</code> · {credentials.orderStatus === "paid"
                    ? "LUNAS — otomatis masuk omzet Owner."
                    : "PENDING — konfirmasi di menu Transaksi setelah transfer masuk."}
                </p>
              )}
