...
import { DinoDivider, DinoFootprints, FloatingLeaves } from "./DinoAccents";
import styles from "./style.module.css";

function DigCountdown({ targetDate }: { targetDate: string }) {
  ...
  return (
    <div className={styles.countdownBand}>
      <p className={styles.eyebrow}>Petualangan Segera Dimulai</p>
...
export default function DinoBirthday({
  invitation,
}: {
    ...
    return (
      <main className={styles.root}>
        <section className={styles.cover}>
          ...
          <img
            className={styles.coverLeaf}
            src="/photos/dinosaur-adventure/leaf.webp"
            alt=""
            aria-hidden="true"
          />
          <div className={styles.coverFrame}><span /></div>
          <DinoFootprints className={`${styles.corner} ${styles.cornerTopLeft}`} />
          <DinoFootprints className={`${styles.corner} ${styles.cornerTopRight}`} mirrored />
          <DinoFootprints className={`${styles.corner} ${styles.cornerBottomLeft}`} mirrored />
          <DinoFootprints className={`${styles.corner} ${styles.cornerBottomRight}`} />
          <motion.div
    ...
    <main className={styles.root}>
      ...
      <section className={`${styles.section} ${styles.hero}`}>
        <img
          ...
          src="/photos/dinosaur-adventure/dino-hero.webp"
          alt=""
          aria-hidden="true"
        />
        <Reveal>
          <p className={styles.eyebrow}>Catatan Ekspedisi</p>
          <h1 className={styles.heroTitle}>
            Petualang kecil kami berulang tahun ke-{invitation.child.age || ""}
          </h1>
          <p className={styles.heroCopy}>
            {invitation.opening.description ||
              "Bersiaplah menjelajah hutan purba! Kami mengundang Anda untuk ikut merayakan hari ulang tahun petualang kecil kami."}
          </p>
        </Reveal>
      ...
      <section className={`${styles.section} ${styles.explorerSection}`}>
        <Reveal>
          <p className={styles.eyebrow}>Profil Penjelajah</p>
          ...
          <div className={styles.eggWrap}>
            ...
            <img
              ...
              src="/photos/dinosaur-adventure/frame.webp"
              alt=""
              aria-hidden="true"
            />
          </div>
          <p className={styles.heroCopy}>
            Penjelajah cilik kesayangan {invitation.parents.father} &amp; {invitation.parents.mother}
          </p>
        </Reveal>
      </section>
      ...
      {event?.rawDate && (
        <div className={styles.countdownScene}>
          <DigCountdown targetDate={event.rawDate} />
          <img
            className={styles.walkingDino}
            src="/photos/dinosaur-adventure/dino-walk.webp"
            alt=""
            aria-hidden="true"
          />
        </div>
      ...
      {event && (
        <section className={styles.section}>
          <Reveal>
            <p className={styles.eyebrow}>Lokasi Ekspedisi</p>
            <h2 className={styles.sectionTitle}>Waktu &amp; Tempat</h2>
            <DinoDivider />
            <article className={styles.eventCard}>
              <p className={styles.eyebrow}>Pesta Petualangan</p>
              <h3>{event.date}</h3>
              <p>{event.time}</p>
              <p>{event.location}</p>
              {invitation.mapsUrl && (
      ...
      {invitation.gallery.length > 0 && (
        <section className={styles.gallerySection}>
          <div className={styles.section}>
            <Reveal>
              <p className={styles.eyebrow}>Jurnal Ekspedisi</p>
      ...
      <footer className={styles.footer}>
        <DinoFootprints className={styles.footerCorner} />
        <Reveal>
          <p className={styles.eyebrow}>Ekspedisi Selesai</p>
          <h2>
            Sampai Jumpa
            <span>di petualangan berikutnya</span>
