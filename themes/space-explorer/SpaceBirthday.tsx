...
import { FloatingStars, SpaceCorner, SpaceDivider } from "./SpaceAccents";
...
function MissionCountdown({ targetDate }: { targetDate: string }) {
  ...
  return (
    <div className={styles.countdownBand}>
      <p className={styles.eyebrow}>T-Minus To Launch</p>
      <h2 className={styles.sectionTitle}>Mission Countdown</h2>
...
export default function SpaceBirthday({
  invitation,
}: {
    ...
    return (
      <main className={styles.root}>
        <section className={styles.cover}>
          ...
          <img
            className={styles.coverPlanet}
            src="/photos/space-explorer/planet.webp"
            alt=""
            aria-hidden="true"
          />
          <div className={styles.coverFrame}><span /></div>
          <SpaceCorner className={`${styles.corner} ${styles.cornerTopLeft}`} />
          <SpaceCorner className={`${styles.corner} ${styles.cornerTopRight}`} mirrored />
          <SpaceCorner className={`${styles.corner} ${styles.cornerBottomLeft}`} mirrored />
          <SpaceCorner className={`${styles.corner} ${styles.cornerBottomRight}`} />
          <motion.div
    ...
    <main className={styles.root}>
      ...
      <section className={`${styles.section} ${styles.hero}`}>
        <img
          ...
          src="/photos/space-explorer/rocket.webp"
          alt=""
          aria-hidden="true"
        />
        <Reveal>
          <p className={styles.eyebrow}>Mission Log 001</p>
          <h1 className={styles.heroTitle}>
            Kapten kecil kami berulang tahun ke-{invitation.child.age || ""}
          </h1>
          <p className={styles.heroCopy}>
            {invitation.opening.description ||
              "Bersiaplah untuk petualangan luar angkasa! Kami mengundang Anda untuk ikut merayakan hari ulang tahun kapten kecil kami."}
          </p>
        </Reveal>
      ...
      <section className={`${styles.section} ${styles.astronautSection}`}>
        <Reveal>
          <p className={styles.eyebrow}>Astronaut Profile</p>
          ...
          <div className={styles.hexWrap}>
            ...
            <img
              ...
              src="/photos/space-explorer/frame.webp"
              alt=""
              aria-hidden="true"
            />
          </div>
          <p className={styles.heroCopy}>
            Astronaut cilik kesayangan {invitation.parents.father} &amp; {invitation.parents.mother}
          </p>
        </Reveal>
      </section>
      ...
      {event?.rawDate && (
        <div className={styles.countdownScene}>
          ...
          <img
            className={styles.launchShuttle}
            src="/photos/space-explorer/shuttle.webp"
            alt=""
            aria-hidden="true"
          />
        </div>
      ...
      {event && (
        <section className={styles.section}>
          <Reveal>
            <p className={styles.eyebrow}>Launch Coordinates</p>
            <h2 className={styles.sectionTitle}>Waktu &amp; Tempat</h2>
            <SpaceDivider />
            <article className={styles.eventCard}>
              <p className={styles.eyebrow}>Launch Party</p>
              <h3>{event.date}</h3>
              <p>{event.time}</p>
              <p>{event.location}</p>
              {invitation.mapsUrl && (
      ...
      <footer className={styles.footer}>
        <SpaceCorner className={styles.footerCorner} />
        <Reveal>
          <p className={styles.eyebrow}>Mission Complete</p>
          <h2>
            Sampai Jumpa
            <span>di misi luar angkasa selanjutnya</span>
