...
function GardenScene({ staged = false, active = true }: { staged?: boolean; active?: boolean }) {
  return (
    <div className={styles.gardenScene} aria-hidden="true">
      ...
      <motion.div className={styles.sceneFlora} initial={staged ? { opacity: 0 } : false} animate={{ opacity: active ? 1 : 0 }} transition={{ delay: staged ? 1.1 : 0, duration: 1 }}/>
    </div>
  );
}
...
function Couple({ invitation }: { invitation: InvitationData }) {
  return <section id="couple" className={styles.couple}><div className={styles.softLandscape} aria-hidden="true"/><Person person={invitation.bride} role="Putri" index={0}/><Person person={invitation.groom} role="Putra" index={1}/><FloralLayer placement="bottom" delay={.4}/></section>;
}
...
function LiveStreaming({ invitation }: { invitation: InvitationData }) {
  const target = invitation.videoUrl || invitation.bride.instagram || invitation.groom.instagram;
  const href = target?.startsWith("http") ? target : target ? `https://instagram.com/${target.replace("@", "")}` : null;
  return <section className={styles.liveStreaming}><FloralLayer placement="frame"/><motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .4 }} transition={{ duration: .8, ease: revealEase }}><span className={styles.liveIcon}>♧</span><h2>Live Streaming</h2><p>Temui kami secara virtual untuk menyaksikan acara pernikahan kami yang akan disiarkan secara langsung.</p>{href && <a href={href} target="_blank" rel="noreferrer">Instagram</a>}</motion.div></section>;
}
...
export default function LuxuryArtJavaHeritage({ invitation }: { invitation: InvitationData }) {
