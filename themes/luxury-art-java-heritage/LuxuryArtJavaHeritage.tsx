...
function Hero({ invitation }: { invitation: InvitationData }) {
  ...
  return <section id="home" className={styles.hero}><Flora position="top"/><motion.div {...reveal} className={styles.heroCopy}><small>The Wedding of</small><ArchPhoto src={invitation.coverImage} alt={`${bride} dan ${groom}`}/><h2>{bride} &amp; {groom}</h2><p>{invitation.events[0]?.date}</p></motion.div><Landscape withHouse/></section>;
}
...
function Live({ invitation }: { invitation: InvitationData }) { const target=invitation.bride.instagram||invitation.groom.instagram; return <section className={styles.live}><motion.div {...reveal}><span>♧</span><h2>Live Streaming</h2><p>Temui kami secara virtual untuk menyaksikan acara pernikahan kami yang insyaaAllah akan disiarkan langsung melalui akun instagram berikut ini.</p>{target&&<a href={`https://instagram.com/${target.replace("@","")}`} target="_blank" rel="noreferrer">◎ {target}</a>}<Landscape withHouse/></motion.div></section> }
