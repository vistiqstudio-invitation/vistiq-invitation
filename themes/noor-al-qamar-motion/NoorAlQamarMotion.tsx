...
const ASSET_ROOT = "/themes/noor-al-qamar-motion";
...
function OpeningHero({ invitation, opened }: { invitation: InvitationData; opened: boolean }) {
  ...
  function revealAfterWhiteFrame() {
    const video = videoRef.current;
    if (!frameReady && video && video.currentTime >= FRAME_REVEAL_AT) {
      setFrameReady(true);
    }
  }
      ...
      <div className={styles.heroCopy}>
        {frameReady && (
          <motion.div
            ...
            animate="visible"
          >
            <motion.p variants={heroCopyItemVariants}>Bismillahirrahmanirrahim</motion.p>
    ...
    <section id="welcome" className={styles.welcomeSection}>
      <SectionReveal className={styles.welcomeInner}>
        <motion.h2 variants={scrollItemVariants} initial="hidden" whileInView="visible" viewport={scrollViewport} custom={0.06}>
          Dengan Memohon<br />Ridho Allah SWT
        </motion.h2>
...
function Countdown({ event }: { event?: InvitationData["events"][number] }) {
  const target = event?.rawDate ? new Date(event.rawDate).getTime() : Number.NaN;
...
export default function NoorAlQamarMotion({ invitation }: { invitation: InvitationData }) {
